...
def train(arguments,ip=0):
    ...
    if arguments.file1 is not None:
        ...
        tmp = None
    for i in range(num):
        env = env_data.env(arguments.address, arguments.host, arguments.server_port*(arguments.auto_ip*-1+1)+ip*arguments.auto_ip, 
                           ...
                           arguments.special_division, arguments.special_division_value, arguments.damage_reset_ball_distance,
        ...
        if args.auto_ip:
            ...
            server = subprocess.Popen(
                ["rcssserver",
                 ...
                 "server::synch_mode=true", "server::free_kick_faults=false", "server::forbid_kick_off_offside=false","server::penalty_shoot_outs=false", "server::half_time=-1",
                 "server::stamina_capacity=-1",
                 "server::drop_ball_time=500", "server::port=" + str(p1),
                 "server::coach_port=" + str(p2), "server::olcoach_port=" + str(p3),"server::auto_mode=true","server::connect_wait=25","server::kick_off_wait=10","server::game_over_wait=10"])
        else:
            server = subprocess.Popen(
                ["rcssserver",
                 "server::game_logging=false", "server::text_logging=false", "server::nr_normal_halfs=1",
                 ...
                 "server::coach_port=" + str(args.server_port + 1), "server::olcoach_port=" + str(args.server_port + 2),"server::auto_mode=true","server::connect_wait=25","server::kick_off_wait=10","server::game_over_wait=10"])
